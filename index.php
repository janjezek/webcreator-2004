...
$vysledek = mysql_query("select c.id,c.nadpis,c.perex,r.id,r.rubrika,a.id,a.jmeno,c.datum from clanky c, rubriky r, autori a where c.id_rubrika = r.id and c.id_autor = a.id and c.stav = 'v' and datum <= $cas order by c.datum desc limit $zaznam, $pocet",$db);
$control = mysql_num_rows($vysledek);

if ($control == "0") {
  if (!isset($_GET["list"])) {
    echo "<p style=\"text-align: center\">Žádné èlánky zatím nebyly vloženy!</p>";
  } else {
